...
export async function POST(request) {
  try {
    ...
    if (!resumeText || resumeText.trim().length < 50) {
      console.log("raw_text missing — re-extracting from storage for MCQ interview generation...");
...
------------------------
    ...
    const generatedData = await generateAIResponse({
      systemPrompt: mcqGeneratorPrompt,
      userPrompt,
      temperature: 0.7,
      maxTokens: 1500,
    });
    ...
    const { data: session, error: sessionDbError } = await supabase
      ...
      .insert({
        ...
        status: 'pending',
        type: 'mcq' // Custom field to denote MCQ interview
      })
      .select()
      .single();
    ...
    console.log("✓ MCQ Interview session created:", session.id);
    return NextResponse.json(session);

  } catch (error) {
    console.error("MCQ Interview Generation Error:", error);
    return NextResponse.json(
