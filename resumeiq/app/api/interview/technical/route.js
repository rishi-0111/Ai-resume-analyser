...
import { technicalInterviewPrompt } from '@/lib/ai/prompts';
...
export async function POST(request) {
  try {
    ...
    if (!resumeText || resumeText.trim().length < 50) {
      console.log("raw_text missing — re-extracting from storage for Technical interview generation...");
...
------------------------
    ...
    const generatedData = await generateAIResponse({
      systemPrompt: technicalInterviewPrompt,
      userPrompt,
      temperature: 0.7,
      maxTokens: 1500,
    });
    ...
    const { data: session, error: sessionDbError } = await supabase
      ...
      .insert({
        ...
        status: 'pending',
        type: 'technical' // Custom field to denote Technical interview
      })
      .select()
      .single();
